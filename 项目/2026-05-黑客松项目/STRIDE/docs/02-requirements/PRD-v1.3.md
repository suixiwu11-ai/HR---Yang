...
| 报告 | 格式 | 说明 |
|------|------|------|
| HR 详版 | Markdown + **HTML** | 含来源附录、分摊矩阵；可浏览器打开或下载 |
| 经营摘要 | Markdown + **HTML** | 与 HR 报告 **同源** 核算快照 |

### 3.5 10× 协作（本期 Lite）

> 业务背景：[business-narrative.md §5.7](../00-background/business-narrative.md) · [10x-team-program.md](../00-background/10x-team-program.md)

| 能力 | 要求 |
|------|------|
| **Handoff 台账** | 登记领域 Pod、绑定产品线、标题、状态（草稿/评审中/已完成） |
| **采纳登记** | 将 Handoff 产出关联到 PL + 岗位族，可链接路线图/API/文档说明 |
| **协作看板** | 分 PL / RF 展示本季支撑完成率、采纳条数（聚合） |
| **预警** | 本期至少实现 **W8**：Handoff 已完成且当季采纳数 = 0（探索期 PL 降敏） |
| **数据交换** | 支持 JSON **导入/导出** 季度包（含 10× 实体）；与核算快照同源 |

**路由**：`/x10`；组织 Tab 提供入口。  
**本期不做**：10× 独立对外 App、Fellowship 全流程 HRIS、W6–W11 全部自动化、协作分自动挂钩薪酬。
...
| 10× 扩展预警 W6–W11 全自动 | 见 [10x-team-assessment-framework.md](./10x-team-assessment-framework.md) |
...
/insight              → 问数（可选全屏）
/scenario             → 情景试算
/x10                  → 10× 协作（Handoff / 采纳 / 协作看板）
/settings             → 演示数据、JSON 导入/导出
/wizard/1 | 2 | 3     → 战略配置 / 组织投入 / 产出证据（本期可仅种子预填）
...
| GET/POST | `/api/x10/handoffs` | Handoff 台账 |
| POST | `/api/x10/adoptions` | 采纳登记 |
| GET | `/api/x10/collaboration` | 协作达标（聚合） |
| POST | `/api/data/import` | 导入季度 JSON 包 |
| GET | `/api/data/export` | 导出季度 JSON 包 |

完整表与技术栈见 **[architecture-v2.1.md](../03-architecture/architecture-v2.1.md)**。
