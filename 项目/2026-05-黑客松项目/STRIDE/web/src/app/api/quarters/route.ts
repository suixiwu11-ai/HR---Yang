...
export async function GET() {
  return NextResponse.json({ quarters: await listQuarters() });
}

export async function POST(req: Request) {
  const body = await req.json();
  const q = await upsertQuarter({
    id: body.id,
    label: body.label,
    weeks: body.weeks,
  ...
  if (body.roleFte && Array.isArray(body.roleFte)) {
    for (const r of body.roleFte as { rf_id: string; fte: number }[]) {
      await setRoleFte(q.id, r.rf_id, r.fte);
    }
  }
  if (body.observations && Array.isArray(body.observations)) {
    for (const o of body.observations as {
      ...
      await setObservation(q.id, o.metric_code, o.value, o.source_type, o.pl_id ?? null);
    }
  }
